...
def efc_custom(technique:str, fig_folder: str, fig_name: str):
    ...
    coupling, h_i, cutoff, _, _ = one_class_fit(np.array(X_train_df), Q, LAMBDA)  # train model
    y_pred, y_energies = one_class_predict(np.array(X_test_df), coupling, h_i, cutoff, Q)  # test model
...
def efc_with_percentage_labeled(technique:str, fig_folder: str, fig_name: str, percentage: int):
    X_train, X_test, y_train, y_test = run_elliptic_preprocessing_pipeline(last_train_time_step=LAST_TRAIN_TIME_STEP,
                                                                           last_time_step=LAST_TIME_STEP,
                                                                           only_labeled=True)
    ...
    y_pred, y_energies = one_class_predict(np.array(X_test), coupling, h_i, cutoff, Q)  # test model
    plot_efc_energies(y_test, y_energies, fig_folder, fig_name, cutoff=cutoff)
    sizes = get_dataset_size(technique=technique, X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test)
    metric_dict = calculate_model_score(technique=technique, y_true=y_test.values, y_pred=y_pred)
    confusion_matrix = custom_confusion_matrix(technique=technique, y_test=y_test, y_pred=y_pred)
