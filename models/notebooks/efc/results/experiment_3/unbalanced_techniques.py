...
def efc_baseline(
    technique: str, fig_folder: str, fig_name: str, only_labeled: bool = True
):
...
def equally_distributing_correlating(
    technique: str, fig_folder: str, fig_name: str, only_labeled: bool = True
):
    X_train, X_test, y_train, y_test = run_elliptic_preprocessing_pipeline(
        last_train_time_step=LAST_TRAIN_TIME_STEP,
    ...
    df = train_test_from_splitted(
        X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, return_df=True
    )
    df = df.sample(frac=1)
...
def smote(
    ...
    X: pd.DataFrame = None,
    y: pd.Series = None,
    test_complete_dataset: bool = False,
    ...
    if fig_folder is None:
        fig_folder = os.path.dirname(os.path.abspath(__file__))

    smote = SMOTE(sampling_strategy="minority", n_jobs=-1)
    X_sm, y_sm = smote.fit_resample(X, y)
    X_train, X_test, y_train, y_test = train_test_from_x_y(X_sm, y_sm)
    if test_complete_dataset:
        if is_resampled:
            _, X_test, _, y_test = train_test_from_x_y(X, y)
        else:
            _, X_test, _, y_test = run_elliptic_preprocessing_pipeline(
                last_train_time_step=LAST_TRAIN_TIME_STEP,
                last_time_step=LAST_TIME_STEP,
                only_labeled=only_labeled,
            )
...
def random_resampling(
    ...
    print(
        "Genuine:",
        y_resampled.value_counts()[0],
        "/",
        round(y_resampled.value_counts()[0] / len(y_resampled) * 100, 2),
        "% of the dataset",
    )
    print(
        "Frauds:",
        ...
        "/",
        round(y_resampled.value_counts()[1] / len(y_resampled) * 100, 2),
        "% of the dataset",
    )
