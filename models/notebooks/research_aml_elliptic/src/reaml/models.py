...
def pyod_predict_scores(
    X_train_df, X_test_df, y_train, model, predict_on="test", semisupervised=False
):
    if semisupervised == True:
        X_train_df = X_train_df[y_train == 0]
...
def get_thresholds_for_all_contamination_levels(contamination_levels, predicted_scores):
    ...
    for contamination in contamination_levels:
        thresholds[contamination] = contamination_to_threshold(
            contamination, predicted_scores
        )
    return thresholds


def pyod_per_contamination_level(
    X_train_df,
    X_test_df,
    y_train,
    ...
    predicted_scores = pyod_predict_scores(
        X_train_df, X_test_df, y_train, model, predict_on, semisupervised
    )
    thresholds = get_thresholds_for_all_contamination_levels(
        contamination_levels, predicted_scores
    )
...
def batch_pyod_per_contamination_level(
    X_train_df,
    X_test_df,
    y_train,
    contamination_levels,
    ...
    for model_name, model in model_dict.items():
        ...
        predictions, predicted_scores = pyod_per_contamination_level(
            X_train_df,
            X_test_df,
            y_train,
            contamination_levels,
...
def supervised_model_per_contamination_level(
    X_train_df, X_test_df, y_train, contamination_levels, model
):
    """Accepts a list of contamination levels and a single model"""
    model.fit(X_train_df, y_train)
    ...
    predictions_at_contamination_levels = {}
    for level, thresh in thresholds.items():
        predictions = predict_based_on_threshold(
            thresh, predicted_scores, formula="greater_or_equal"
        )
        if level == 0:
            predictions = predict_based_on_threshold(
                thresh, predicted_scores, formula="greater"
            )
        predictions_at_contamination_levels[level] = predictions
    return predictions_at_contamination_levels, predicted_scores
