...
def calc_model_performance_over_time(
    X_test_df,
    y_test,
    contamination_levels_subset,
    scoring="f1",
    aggregated_timestamp_column="time_step",
    **model_predictions,
):
    ...
    for contamination_level in contamination_levels_subset:
        for model_name, predictions in model_predictions.items():
            ...
            for time_step in range(first_test_time_step, last_time_step + 1):
                time_step_idx = np.flatnonzero(
                    X_test_df[aggregated_timestamp_column] == time_step
                )
                y_true = y_test[X_test_df[aggregated_timestamp_column] == time_step]
                y_pred = [y_pred_[i] for i in time_step_idx]

                model_scores.append(
                    calculate_model_score(y_true.astype("int"), y_pred, scoring)
                )
...
def calc_average_score_and_std_per_timestep(
    X_test_df, y_test, y_preds, aggregated_timestamp_column="time_step", scoring="f1"
):
    ...
    for y_pred in y_preds:
        ...
        for time_step in range(last_train_time_step + 1, last_time_step + 1):
            ...
            model_scores.append(
                calculate_model_score(y_true_ts.astype("int"), y_pred_ts, scoring)
            )
        all_model_scores.append(model_scores)

    avg_f1 = np.array(
        [np.mean([f1_scores[i] for f1_scores in all_model_scores]) for i in range(15)]
    )
    std = np.array(
        [np.std([f1_scores[i] for f1_scores in all_model_scores]) for i in range(15)]
    )
