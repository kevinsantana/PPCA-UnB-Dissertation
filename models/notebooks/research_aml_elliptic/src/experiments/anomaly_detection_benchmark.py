...
predictions_at_contamination_levels, _ = supervised_model_per_contamination_level(
    X_train_df, X_test_df, y_train, contamination_levels, random_forest
)

model_predictions["Random Forest"] = predictions_at_contamination_levels


model_stats_df = metric_per_contamination_level(
    y_true=y_test, metric="f1", model_predictions=model_predictions
)

f1_per_contamination_level = model_stats_df.pivot(
    index="model", columns="contamination_level", values="f1"
)
...
f1_per_contamination_level_reduced.to_csv(
    os.path.join(
        ROOT_DIR,
        "research_aml_elliptic/output/table_2_illicit_f1_per_contamination_level.csv",
    )
)
