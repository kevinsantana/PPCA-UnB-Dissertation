...
embedding_df["prediction"] = [
    "Illicit" if pred == 1 else "Licit"
    for pred in model_predictions[model][contamination_level]
]
...
plot_UMAP_projection(
    ...
    savefig_path=os.path.join(
        ROOT_DIR, "research_aml_elliptic/output/figure_3_umap_predicted_label.png"
    ),
)
...
plot_UMAP_projection(
    ...
    palette=["cadetblue", "coral"],
    savefig_path=os.path.join(
        ROOT_DIR, "research_aml_elliptic/output/figure_4_umap_true_label.png"
    ),
)
